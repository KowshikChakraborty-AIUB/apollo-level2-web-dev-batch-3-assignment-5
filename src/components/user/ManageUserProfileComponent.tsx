...
import { useDeleteUserMutation, useGetUserByEmailIdQuery, useUpdateUserByEmailIdMutation } from "@/redux/api/authApi/authApi";

const ManageUserProfileComponent = (props: any) => {
    const dispatch = useAppDispatch();
    const { params: emailId } = props;
    ...
    const onSubmit = async (values: z.infer<typeof profileUpdateValidationSchema>) => {
        ...
        try {
            ...
            if (res?.data?.success) {
                navigate(location?.state ? location.state : '/userProfile', { replace: true });
    ...
    return (
        <div>
            <div className="w-full md:w-3/4 lg:w-3/5 my-20 mx-auto bg-purple-200 px-6 py-6 rounded-md">
                <div className="w-4/5 mx-auto">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            />
                            <Button type="submit" className="text-center bg-yellow-400 text-black hover:bg-yellow-500  transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300">Update</Button>
                        </form>
                    </Form>
                    {/* <div className="mt-4">
                    ...
                    <button className="btn mt-4 bg-red-500 text-white hover:bg-red-500 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300" onClick={() => (document?.getElementById('my_modal_5') as HTMLDialogElement).showModal()}>Delete Profile</button>
                        ...
                        <div className="modal-box">
                            ...
                            <div className="modal-action">
                                <form method="dialog">
                                    ...
                                    <Button onClick={() => handleDeleteProfile(userData?.data?._id)} className="text-center bg-red-500 text-white hover:bg-red-500 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 mr-4">Delete</Button>
